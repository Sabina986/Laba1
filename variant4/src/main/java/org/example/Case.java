    ...
    public static void update(Scanner sc) {
        try (Connection connection = MysqlConfig.getConnection()) {
            ...
            try (PreparedStatement psInsert = connection.prepareStatement(printSQL)) {
                ...
                while (rs.next()) {
                    String updateSQL = "UPDATE " + tbName + " SET Verh = ?, Niz = ? WHERE Str1 = ?";
                    try (PreparedStatement psUpdate = connection.prepareStatement(updateSQL)) {
                        String num = rs.getString("Str1");
                        String Prov11 = rs.getString("Str1").toLowerCase(); String Prov12 = rs.getString("Str1").toUpperCase();
                        String Prov21 = rs.getString("Str2").toLowerCase(); String Prov22 = rs.getString("Str2").toUpperCase();
                        String Prov1 = Prov11 + "\n" + Prov21; String Prov2 = Prov12 + "\n" + Prov22;
                        psUpdate.setString(1, Prov2);
                        psUpdate.setString(2, Prov1);
                        psUpdate.setString(3, num);
                        int rowsAffected = psUpdate.executeUpdate();
                        if (rowsAffected > 0) {
                            System.out.println("Регистры строк '" + num + "'\n             и '" + rs.getString("Str2") + "'\nуспешно обновлены на");
                            System.out.println("               '" + Prov2 + "'\n             и '" + Prov1 + "'\n");
                        } else {
                            System.out.println("Строки не найдена.");
                        }
